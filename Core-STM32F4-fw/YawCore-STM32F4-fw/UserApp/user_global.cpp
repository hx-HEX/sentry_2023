...
void CompetitionStateUpdate(void) {
    if (G_referee.GameRobotStatus.robot_id == ROBOT_ID_SENTRY_RED) {
        ...
        if(G_referee.GameStatus.game_progress == 4 && !G_sentry.balance_infantry_flag)
        {
            ...
            if(G_referee.GameRobotHP.blue_3_robot_HP == 300)   G_sentry.balance_infantry_num += 3;
            if(G_referee.GameRobotHP.blue_4_robot_HP == 300)   G_sentry.balance_infantry_num += 4;
            if(G_referee.GameRobotHP.blue_5_robot_HP == 300)   G_sentry.balance_infantry_num += 5;
        ...
        {
            G_sentry.balance_infantry_flag = true;
            if(G_referee.GameRobotHP.red_3_robot_HP == 300)   G_sentry.balance_infantry_num += 3;
            if(G_referee.GameRobotHP.red_4_robot_HP == 300)   G_sentry.balance_infantry_num += 4;
            if(G_referee.GameRobotHP.red_5_robot_HP == 300)   G_sentry.balance_infantry_num += 5;
        }
        if(G_referee.GameStatus.game_progress != 4)
        {
            G_sentry.balance_infantry_flag = false;
...
void RobotStatesUpdate(void)
{
    ...
    if(G_referee.RobotCommand.commd_keyboard == PRESS_V)  G_sentry.safe_flag = false;
      ...
      // gun shift strategy
    if((G_referee.PowerHeatData.shooter_id1_17mm_cooling_heat > G_referee.GameRobotStatus.shooter_id1_17mm_cooling_limit - 40) 
    && G_referee.GameRobotStatus.shooter_id1_17mm_cooling_limit != 0 )
    {
        G_sentry.left_stop_shoot_flag = 1;
    }
    if(G_sentry.left_stop_shoot_flag) G_sentry.m_left_stop_shoot_cnt ++;
    ...
    if((G_referee.PowerHeatData.shooter_id2_17mm_cooling_heat > G_referee.GameRobotStatus.shooter_id2_17mm_cooling_limit - 40) 
    && G_referee.GameRobotStatus.shooter_id2_17mm_cooling_limit != 0 )
    {
        G_sentry.right_stop_shoot_flag = 1;
    }
    if(G_sentry.right_stop_shoot_flag) G_sentry.m_right_stop_shoot_cnt ++;
...
void GimbalTargetsUpdate(void)
{
    ...
    {
        if(G_djirc.channel.Ch0 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD) && G_djirc.channel.Ch1 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD) 
            && G_djirc.channel.Ch2 > (RC_CH_VALUE_MAX - RC_CH_VALUE_DEAD)  && G_djirc.channel.Ch3 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD)
            && change_flag_1)
        {
            mannal_delay_cnt++;
            if(mannal_delay_cnt > 2000) 
            {
                mannal_change_flag = 1;
                change_flag_1 = 0;
            }
        }
        else if(!(G_djirc.channel.Ch0 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD) && G_djirc.channel.Ch1 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD) 
            && G_djirc.channel.Ch2 > (RC_CH_VALUE_MAX - RC_CH_VALUE_DEAD)  && G_djirc.channel.Ch3 < (RC_CH_VALUE_MIN + RC_CH_VALUE_DEAD)))
        {
            mannal_delay_cnt = 0;
            change_flag_1 = 1;
...
void ShootTargetsUpdate(void)
{
    ...
    {
        if(G_sentry.gimbalrc_mode == SentryRobot::LEFT_MANNAL || G_sentry.gimbalrc_mode == SentryRobot::LEFT_AUTO)
        {   
        ...
        else if(G_sentry.gimbalrc_mode == SentryRobot::RIGHT_MANNAL || G_sentry.gimbalrc_mode == SentryRobot::RIGHT_AUTO)
        {
            if (G_djirc.channel.Ch4 - RC_CH_VALUE_OFFSET < -RC_CH_VALUE_DEAD)
            {
    ...
    if (G_sentry.shoot_mode == SentryRobot::SHOOT_MANNAL && G_sentry.left_shoot_insurance == false && !G_sentry.left_stop_shoot_flag )
    {
    ...
    else if (G_sentry.shoot_mode == SentryRobot::SHOOT_MANNAL && G_sentry.right_shoot_insurance == false && !G_sentry.right_stop_shoot_flag)
    {
        if (G_djirc.channel.Ch3 - RC_CH_VALUE_OFFSET < -RC_CH_VALUE_DEAD)
        {
