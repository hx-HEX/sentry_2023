...
void User_SysTickCallback(void){
    ...
    if(G_system_monitor.SysTickTime % 1000 == 0) {
		// Main Task Monitor
		DETECT_MONITOR(YawGimbalDataSendTask);
		DETECT_MONITOR(SystemMonitorTask);
		DETECT_MONITOR(RobotControlTask);
		DETECT_MONITOR(RightYawDataSendTask);
		DETECT_MONITOR(LeftGimbalDataSendTask);
		DETECT_MONITOR(ChassisCoreDataSendTask);
		DETECT_MONITOR(NavigationDataSendTask);
		DETECT_MONITOR(Dijrc_Receive);
		DETECT_MONITOR(Chassis_Core_Receive);
		DETECT_MONITOR(Yaw_Gimbal_Receive);
		DETECT_MONITOR(Right_Gimbal_Receive);
		DETECT_MONITOR(Navigation_Receive);
		DETECT_MONITOR(Left_Gimbal_Receive);
...
void User_UART_RX_Callback(USART_TypeDef* USARTx){
	...
	else if (G_chassis_core.usart_chassis_core->USART_RT.USARTx == USARTx){
		Usartx = G_chassis_core.usart_chassis_core;
	}
	else if (G_yaw_gimbal.usart_yaw_gimbal->USART_RT.USARTx == USARTx){
		Usartx = G_yaw_gimbal.usart_yaw_gimbal;
	}
	else if (G_right_gimbal.usart_right_gimbal->USART_RT.USARTx == USARTx){
		Usartx = G_right_gimbal.usart_right_gimbal;
	}
	else if (G_navigation.usart_navigation->USART_RT.USARTx == USARTx){
		Usartx = G_navigation.usart_navigation;
	}
	else if (G_left_gimbal.usart_left_gimbal->USART_RT.USARTx == USARTx){
		Usartx = G_left_gimbal.usart_left_gimbal;
	}
	...
	if(USART_GetITStatus(Usartx->USART_RT.USARTx, USART_IT_IDLE)!= RESET){
		...
		if(G_chassis_core.usart_chassis_core->USART_RT.USARTx == USARTx){
