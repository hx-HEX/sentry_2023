...
void Mahony::AngleUpdate(const float acc[3], const float gyro[3])
{
    ...
    float esti_acc_x, esti_acc_y, esti_acc_z;
    ...
    if (!m_first_in_flag && ((acc[0] != 0.0f) || (acc[1] != 0.0f) || (acc[2] != 0.0f))) {
        ...
        error_acc_x = real_acc_y * esti_acc_z - real_acc_z * esti_acc_y;
        error_acc_y = real_acc_z * esti_acc_x - real_acc_x * esti_acc_z;
        error_acc_z = real_acc_x * esti_acc_y - real_acc_y * esti_acc_x;
    ...
    esti_acc_x = 2 * calcu_temp1;
    esti_acc_y = 2 * calcu_temp2;
    esti_acc_z = 1 - 2 * calcu_temp3;
    
    // Update current euler angles (use updated quaternion)
    m_eular_angle.pitch = -asin(esti_acc_x) * RADIAN2DEGREE_VALUE;
    m_eular_angle.roll = atan2f(calcu_temp2, esti_acc_z) * RADIAN2DEGREE_VALUE;
    m_eular_angle.yaw = atan2f(calcu_temp4, calcu_temp5) * RADIAN2DEGREE_VALUE;
