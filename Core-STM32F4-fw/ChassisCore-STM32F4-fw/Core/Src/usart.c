...
void USART2_Configuration(void)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_DMA1,ENABLE);//使能PA端口时钟
    ...
    USART2_InitStructure.USART_BaudRate = 115200;//波特率
   ...
   DMA_InitStructure.DMA_Memory0BaseAddr    = (uint32_t)UA2RxDMAbuf;//内存地址
...
void USART3_Configuration(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_DMA1,ENABLE);//使能PB端口时钟
   ...
   DMA_InitStructure.DMA_Memory0BaseAddr    = (uint32_t)UA3RxDMAbuf;//内存地址
   DMA_InitStructure.DMA_DIR                = DMA_DIR_PeripheralToMemory;//DMA传输为单向
...
void UART5_Configuration(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC | RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_DMA1,ENABLE);//使能PC端口时钟
	...
	USART5_InitStructure.USART_BaudRate = 460800;//波特率
	...
	USART5_InitStructure.USART_Mode = USART_Mode_Rx|USART_Mode_Tx;//仅接收
	USART5_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;//无硬件数据流控制
	USART_Init(UART5,&USART5_InitStructure);//初始化串口
	...
	DMA_InitStructure.DMA_DIR                   = DMA_DIR_PeripheralToMemory;//设置数据传输方向
	DMA_InitStructure.DMA_BufferSize            = UART5_RXDMA_LEN;//设置DMA一次传输数据量的大小
	DMA_InitStructure.DMA_PeripheralInc         = DMA_PeripheralInc_Disable;//设置外设地址不变
	DMA_InitStructure.DMA_MemoryInc             = DMA_MemoryInc_Enable;	//设置内存地址递增
	DMA_InitStructure.DMA_PeripheralDataSize    = DMA_PeripheralDataSize_Byte;//设置外设的数据长度为字节（8bits）
	DMA_InitStructure.DMA_MemoryDataSize        = DMA_MemoryDataSize_Byte;//设置内存的数据长度为字节（8bits）
...
void USART6_Configuration(void)
{
   ...
   DMA_InitStructure.DMA_Memory0BaseAddr    = (uint32_t)UA6RxDMAbuf;//内存地址
   DMA_InitStructure.DMA_DIR                = DMA_DIR_PeripheralToMemory;//DMA传输为单向
